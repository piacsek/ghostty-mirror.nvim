...
Ghostty follows the background, foreground, and cursor color of *any*
colorscheme with no extra config, since those come straight from its `Normal`
and `Cursor` highlights. The 16-color ANSI `palette` is only mirrored when the
colorscheme sets its own `g:terminal_color_*` (most do; some, like catppuccin,
gate it behind a `term_colors` option). For pixel-perfect ANSI colors on a
scheme that doesn't, drop a hand-made file at
`~/.config/ghostty/themes/<colorscheme>` (the bundled skill can author one).

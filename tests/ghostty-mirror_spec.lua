		...
		it("skips the tmux source and reload when the tmux pointer is unchanged", function()
			with_palette(function()
				with_tmp_dir(function(dir)
					...
					local mirror = fresh_require()
					mirror.setup({
						themes_dir = g, theme_file = dir .. "/g-current", reload_command = { "echo" },
						tmux = { enabled = true, themes_dir = t, theme_file = dir .. "/t-current.conf", reload_command = { "echo" } },
					})
					mirror.push("mytheme")
					restore()
					assert.equals(0, #calls)
		...
		it("push_tmux is a no-op for an empty colorscheme name", function()
			with_palette(function()
				with_tmp_dir(function(dir)
					...
					local calls, restore = stub_system()
					local mirror = fresh_require()
					mirror.setup({ tmux = { enabled = true, themes_dir = t, theme_file = dir .. "/tc.conf", reload_command = { "echo" } } })
	...
	describe("sync_on_startup", function()
		it("applies the theme named in theme_file on setup", function()
			with_tmp_dir(function(dir)
				...
				local _, restore = stub_system()
				local mirror = fresh_require()
				mirror.setup({ themes_dir = dir .. "/themes", theme_file = theme_file, reload_command = { "echo" }, generate = false, debounce_ms = 0, sync_on_startup = true })
		...
		it("does not apply any theme on setup by default", function()
			with_tmp_dir(function(dir)
				...
				local mirror = fresh_require()
				mirror.setup({ themes_dir = dir .. "/themes", theme_file = dir .. "/tc", reload_command = { "echo" }, generate = false, debounce_ms = 0 })
				restore()
	...
	describe("sync_on_focus", function()
		it("applies the theme_file theme on FocusGained when it differs from the loaded one", function()
			with_tmp_dir(function(dir)
				local _, restore = stub_system()
				local mirror = fresh_require()
				mirror.setup({ themes_dir = dir .. "/themes", theme_file = dir .. "/tc", reload_command = { "echo" }, generate = false, debounce_ms = 0, sync_on_focus = true })
				vim.cmd.colorscheme("default")
				vim.fn.writefile({ "theme = elflord" }, dir .. "/tc")

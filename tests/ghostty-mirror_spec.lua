	...
	describe("generate_tmux", function()
		...
		it("blends the status bar background from Normal toward the accent", function()
			with_palette(function()
				...
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true, accent_ansi = 5, bar_blend = 0.25 } })
		...
		it("picks the accent foreground by contrast: light on a dark accent, dark on a light one", function()
			with_palette(function()
				vim.g.terminal_color_5 = "#d700ff" -- dark-ish accent -> light fg (Normal.fg)
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true, accent_ansi = 5 } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				assert.is_truthy(joined:find('window%-status%-current%-style "bg=#d700ff,fg=#cdd6f4'))

				vim.g.terminal_color_5 = "#eeeeee" -- light accent -> dark fg (Normal.bg)

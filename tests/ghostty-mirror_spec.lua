...
describe("ghostty-mirror", function()
	...
	describe("generate_tmux", function()
		...
		it("mirrors the copy-mode selection from Visual, not the accent, by default", function()
			with_palette(function()
				-- Visual bg #45475a (dark) -> contrast fg is the light Normal.fg.
				-- Accent (Type) is #89b4fa, so the two are deliberately distinct.
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#45475a,fg=#cdd6f4'))
			end)
		end)

		it("sources the copy-mode selection from the configured selection_hl group", function()
			with_palette(function()
				vim.api.nvim_set_hl(0, "IncSearch", { bg = 0x303040, fg = 0xffd7af })
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true, selection_hl = "IncSearch" } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#303040,fg=#ffd7af'))
			end)
		end)

		it("contrast-picks the selection fg when the selection_hl group has no foreground", function()
			with_palette(function()
				-- A light selection bg with no fg -> dark contrast fg (Normal.bg).
				vim.api.nvim_set_hl(0, "Visual", { bg = 0xd0d0d0 })
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#d0d0d0,fg=#1e1e2e'))
			end)
		end)

		it("falls back to the accent for the copy-mode selection when selection_hl has no background", function()
			with_palette(function()
				vim.api.nvim_set_hl(0, "Visual", { reverse = true }) -- no bg to mirror
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				-- accent_pair: bg=#89b4fa (Type), fg=#1e1e2e (contrast)
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#89b4fa,fg=#1e1e2e'))
			end)
		end)
	end)
	...
	describe("generate_tmux: overrides", function()
		...
		it("a selection override replaces the selection_hl-derived copy-mode selection", function()
			with_palette(function()
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true, overrides = { mytheme = { selection = "#402030" } } } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				-- #402030 is dark -> contrast fg is the light Normal.fg
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#402030,fg=#cdd6f4'))
			end)
		end)

		it("falls back to the selection_hl-derived selection when the override color is invalid", function()
			with_palette(function()
				local mirror = fresh_require()
				mirror.setup({ tmux = { enabled = true, overrides = { mytheme = { selection = "nope" } } } })
				local joined = table.concat(mirror.generate_tmux("mytheme"), "\n")
				assert.is_truthy(joined:find('set %-g mode%-style "bg=#45475a'))
			end)
		end)

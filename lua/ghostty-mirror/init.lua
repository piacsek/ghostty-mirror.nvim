...
---@param name string
---@return boolean
local function valid_name(name) return name:match("^[%w._%-]+$") ~= nil and name ~= "." and name ~= ".." end

---Write lines to a path, refusing to write through a symlink: a pre-planted
---link would silently redirect a plugin write into an arbitrary file. Returns
---whether the write happened — fail silently, never wrongly.
---@param lines string[]
---@param path string
---@return boolean
local function write_no_symlink(lines, path)
	local st = vim.uv.fs_lstat(path)
	if st and st.type == "link" then return false end
...
function M.resolve(colorscheme)
	...
	if light then
		...
		if vim.uv.fs_stat(variant) and usable(variant, colorscheme .. cfg.light_variant_suffix) then
			return colorscheme .. cfg.light_variant_suffix
		end
	end
